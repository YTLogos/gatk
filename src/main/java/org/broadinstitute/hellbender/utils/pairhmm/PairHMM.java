...
public abstract class PairHMM implements Closeable{
    ...
    // -----------------------------------------------------------------------------------------------
    // fields, initializers and clean up
    // -----------------------------------------------------------------------------------------------
     ...
     * @param haplotype2 the second haplotype2
     *
     ...
     */
    public static int findFirstPositionWhereHaplotypesDiffer(final byte[] haplotype1,
                                                             final byte[] haplotype2)
    ...
    /**
     * To be implemented by subclasses to do calculation for
     * {@link #computeReadLikelihoodGivenHaplotypeLog10(byte[], byte[], byte[], byte[], byte[], byte[], boolean, byte[])}.
     */
    ...
    private static int findMaxAlleleLength(final List<? extends Allele> alleles) {
        final OptionalInt max = alleles.stream().mapToInt(Allele::length).max();
