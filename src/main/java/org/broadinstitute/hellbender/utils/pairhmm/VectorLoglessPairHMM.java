...
public final class VectorLoglessPairHMM extends LoglessPairHMM {
    ...
    public VectorLoglessPairHMM(Implementation implementation) throws UserException.HardwareFeatureException {
        final boolean isSupported;

        // TODO: Connect PairHMMNativeArguments to command line arguments
        PairHMMNativeArguments pairHmmArgs = new PairHMMNativeArguments();
        pairHmmArgs.maxNumberOfThreads = 1;
        pairHmmArgs.useDoublePrecision = false;

        switch (implementation) {
            case AVX:
                isSupported = new IntelPairHmm().load();
                if (!isSupported) {
            ...
            case OMP:
                isSupported = new IntelPairHmmOMP().load();
                if (!isSupported) {
                    throw new UserException.HardwareFeatureException("Machine does not support OpenMP AVX PairHMM.");
                }
                pairHmm = new IntelPairHmmOMP();
                logger.info("Maximum number of OpenMP threads for AVX : " + pairHmmArgs.maxNumberOfThreads);
                break;
