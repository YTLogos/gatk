...
public final class QualityUtilsUnitTest extends BaseTest {
    ...
    public void testPhredScaleLog10CorrectRate(){
        Assert.assertEquals(QualityUtils.phredScaleLog10CorrectRate(StrictMath.log10(0.9)),    10.0, 1e-9);
        Assert.assertEquals(QualityUtils.phredScaleLog10CorrectRate(StrictMath.log10(0.99)),   20.0, 1e-9);
        Assert.assertEquals(QualityUtils.phredScaleLog10CorrectRate(StrictMath.log10(0.999)),  30.0, 1e-9);
        Assert.assertEquals(QualityUtils.phredScaleLog10CorrectRate(StrictMath.log10(0.9999)), 40.0, 1e-9);
    }
    ...
    public Object[][] makeMyDataProvider() {
        ...
        for ( int qual = 0; qual < 255; qual++ ) {
            tests.add(new Object[]{(byte)(qual & 0xFF), StrictMath.pow(10.0, ((double)qual)/-10.0)});
        }
    ...
    public void testMyData(final byte qual, final double errorRate) {
        ...
        Assert.assertEquals(actualLog10ErrorRate, StrictMath.log10(errorRate), TOLERANCE);
        final double actualLog10TrueRate = QualityUtils.qualToProbLog10(qual);
        Assert.assertEquals(actualLog10TrueRate, StrictMath.log10(trueRate), TOLERANCE);

        // test that we can convert our error rates to quals, accounting for boundaries
        final int expectedQual = StrictMath.max(StrictMath.min(qual & 0xFF, QualityUtils.MAX_SAM_QUAL_SCORE), 1);
        ...
        for ( int maxQual = 10; maxQual < QualityUtils.MAX_SAM_QUAL_SCORE; maxQual++ ) {
            ...
            final byte expectedQual2 = (byte)(StrictMath.max(StrictMath.min(qual & 0xFF, maxQual), 1) & 0xFF);
    ...
    public void testLogProbToPhred() {
        for (final double logProb : Arrays.asList(-10.0, -5.0, -1.0, 0.0, 0.5, 1.0)) {
            final double prob = StrictMath.exp(logProb);
            final double phred = QualityUtils.logProbToPhred(logProb);
            Assert.assertEquals(prob, StrictMath.pow(10, -phred/10), TOLERANCE);
        }
    }
    ...
    public void testQualCaches() {
        ...
        Assert.assertEquals(QualityUtils.qualToErrorProbLog10((byte) 20), StrictMath.log10(0.01), 1e-6);
        Assert.assertEquals(QualityUtils.qualToProb((byte) 20), 0.99, 1e-6);
        Assert.assertEquals(QualityUtils.qualToProbLog10((byte) 20), StrictMath.log10(0.99), 1e-6);

        Assert.assertEquals(QualityUtils.qualToErrorProb((byte) 30), 0.001, 1e-6);
        Assert.assertEquals(QualityUtils.qualToErrorProbLog10((byte) 30), StrictMath.log10(0.001), 1e-6);
        Assert.assertEquals(QualityUtils.qualToProb((byte) 30), 0.999, 1e-6);
        Assert.assertEquals(QualityUtils.qualToProbLog10((byte) 30), StrictMath.log10(0.999), 1e-6);

        Assert.assertEquals(QualityUtils.qualToErrorProb((byte) 40), 0.0001, 1e-6);
        Assert.assertEquals(QualityUtils.qualToErrorProbLog10((byte) 40), StrictMath.log10(0.0001), 1e-6);
        Assert.assertEquals(QualityUtils.qualToProb((byte) 40), 0.9999, 1e-6);
        Assert.assertEquals(QualityUtils.qualToProbLog10((byte) 40), StrictMath.log10(0.9999), 1e-6);
    }

    @Test()
    public void testBoundingDefault() {
        for ( int qual = 0; qual < 1000; qual++ ) {
            final byte expected = (byte)StrictMath.max(StrictMath.min(qual, QualityUtils.MAX_SAM_QUAL_SCORE), 1);
    ...
    public void testBoundingWithMax() {
        for ( int max = 10; max < 255; max += 50 ) {
            for ( int qual = 0; qual < 1000; qual++ ) {
                final int expected = StrictMath.max(StrictMath.min(qual, max), 1);
    ...
    public Object[][] makePhredDoubleTest() {
        ...
        tests.add(new Object[]{0.0, -10 * StrictMath.log10(Double.MIN_VALUE)});
        tests.add(new Object[]{1.0, 0.0});
        for ( int pow = 1; pow < 20; pow++ ) {
            tests.add(new Object[]{StrictMath.pow(10.0, -1.0 * pow), pow * 10});
            tests.add(new Object[]{StrictMath.pow(10.0, -1.5 * pow), pow * 15});
        }
    ...
    public void testQualToErrorProbDouble() {
        for ( double qual = 3.0; qual < 255.0; qual += 0.1 ) {
            final double expected = StrictMath.pow(10.0, qual / -10.0);
