...
public final class SingleDiploidSampleBiallelicSVGenotyperSparkUnitTest extends BaseTest{
    ...
    public void testGatherAsmIDs(){
        final Set<Long> asmIDs = GenotyperInputParser.gatherAsmIDAnnotations(vcs);
    ...
    public void testMapAssemblyID2ItsAlignments() throws IOException{
        ...
        final Map<Long, List<LocalAssemblyContig>> result = GenotyperInputParser.mapAssemblyID2ItsAlignments(asmId2Contigs, asmId2Alignments, Collections.singleton(9L));
    ...
    public void testFilterJunctionsBasedOnIntervalKillList(){
        ...
        final List<SVJunction> junctions = vcs.stream().map(vc -> SVJunction.convertToSVJunction(vc, ctx.broadcast(assemblyID2assembleContents), ctx.broadcast(referenceMultiSource))).collect(Collectors.toList());
        final Predicate<SVJunction> filter = new GenotyperInputParser.JunctionFilterBasedOnInterval();

        GenotyperInputParser.filterJunctions(junctions, filter);
        Assert.assertEquals(junctions.size(), 5);
        final Predicate<SVJunction> anotherFilter = new GenotyperInputParser.JunctionFilterBasedOnInterval(Collections.singleton(new SimpleInterval("20", 26211000, 26211000)));
        GenotyperInputParser.filterJunctions(junctions, anotherFilter);
        Assert.assertEquals(junctions.size(), 3);
    }
    ...
    public void testGetAssemblyToJunctionsMapping(){
        ...
        final List<SVJunction> junctions = vcs.stream().map(vc -> SVJunction.convertToSVJunction(vc, ctx.broadcast(assemblyID2assembleContents), ctx.broadcast(referenceMultiSource))).collect(Collectors.toList());
        final Map<Long, List<SVJunction>> assemblyToJunctionsMapping = GenotyperInputParser.getAssemblyToJunctionsMapping(junctions);
    ...
    public void testGetFASTQReads(){
        ...
        final List<SVJunction> svJunctions = vcs.stream().map(vc -> SVJunction.convertToSVJunction(vc, ctx.broadcast(assemblyID2assembleContents), ctx.broadcast(referenceMultiSource))).collect(Collectors.toList());
        final JavaPairRDD<Long, List<SVJunction>> asmid2SVJunctions =
                ctx.parallelizePairs(GenotyperInputParser.getAssemblyToJunctionsMapping(svJunctions).entrySet().stream().map(e -> new Tuple2<>(e.getKey(), e.getValue())).collect(Collectors.toList()));

        Map<SVJunction, List<GATKRead>> fastqReads = GenotyperInputParser.associateJunctionWithFastqReads(fastq, asmid2SVJunctions).collectAsMap();
    ...
    public void testGetBamReads(){
        ...
        final Map<SVJunction, List<GATKRead>> result = GenotyperInputParser.associateJunctionWithBamReads(reads, svJunctions).collectAsMap();
    ...
    public void testReadsMergingByName(){
        ...
        final List<GATKRead> merged = GenotyperInputParser.mergeReadsByName(listOne, listTwo);
    ...
    public void testInferGenotypeFromPL(){
        ...
        Genotype gt = SVJunction.inferGenotypeFromPL(builder.make(), dummyAlleles);
        ...
        gt = SVJunction.inferGenotypeFromPL(builder.make(), dummyAlleles);
    ...
    public void testCollectAndSortGenotypedVC(){
        ...
        final List<VariantContext> sortedGenotypedVCs = GenotyperOutputWriter.collectAndSortGenotypedVC(input, referenceMultiSource.getReferenceSequenceDictionary(null));
    ...
    @Test
    public void testWriteGenotypedVariantsToVCF() throws IOException{
        ...
        try(final OutputStream outputStream = new FileOutputStream(tempGenotypedVCF)){
            GenotyperOutputWriter.writeGenotypedVariantsToVCF(sortedGenotypedVCs, referenceMultiSource.getReferenceSequenceDictionary(null), outputStream);
        }

        try(final VCFFileReader reader = new VCFFileReader(tempGenotypedVCF, false)){
            final List<VariantContext> vcsFromFile = Utils.stream(reader).collect(Collectors.toList());
